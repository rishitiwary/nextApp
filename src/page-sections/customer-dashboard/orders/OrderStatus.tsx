...
type Status = "packaging" | "shipping" | "pending" | "completed";

export default function OrderStatus(response:any) {

const resp=!!response && response.response?.data;

  const width = useWindowSize();
  const orderStatus: Status = !!resp && resp.status;
  const stepIconList = ["package-box", "truck-1", "delivery"];
  const orderStatusList = ["packed", "shipped", "pending", "completed"];
    ...
    <Card p="2rem 1.5rem" mb="30px" borderRadius={8}>
      ...
      <FlexBox justifyContent={width < breakpoint ? "center" : "flex-end"}>
        <Typography
          ...
          borderRadius="300px"
          color="primary.main">
          Estimated Delivery Date <b>{resp?.dateTime}</b>
        </Typography>
      </FlexBox>
    </Card>
  );
